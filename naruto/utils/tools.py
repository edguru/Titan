...
_LOG = naruto.logging.getLogger(__name__)
...
def get_file_id_and_ref(message: 'naruto.Message') -> Tuple[Optional[str], Optional[str]]:
...
def post_to_telegraph(a_title: str, content: str) -> str:
    ...
    post_page = post_client.post(
        title=a_title,
        author=auth_name,
        author_url="https://t.me/thenaruto",
        text=content
    )
...
async def take_screen_shot(video_file: str, duration: int, path: str = '') -> Optional[str]:
    ...
    ttl = duration // 2
    thumb_image_path = path or os.path.join(naruto.Config.DOWN_PATH, f"{basename(video_file)}.jpg")
