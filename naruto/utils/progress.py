...
async def progress(current: int,
                   total: int,
                   message: 'naruto.Message',
                   ud_type: str,
                   file_name: str = '',
                   delay: int = naruto.Config.EDIT_SLEEP_TIMEOUT) -> None:
    """ progress function """
    ...
    if (now - last) >= delay:
        ...
        progress_str = progress_str.format(
            ud_type,
            file_name,
            ''.join((naruto.Config.FINISHED_PROGRESS_STR
                     for i in range(floor(percentage / 5)))),
            ''.join((naruto.Config.UNFINISHED_PROGRESS_STR
